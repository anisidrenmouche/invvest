...
export default function Dashboard() {
  ...
  function addPosition(p: Position) {
    ...
    setState(ns); saveState(ns, userId); toast('Position ajoutee !')
  }
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', gap: 14 }}>
            <div style={{ flex: 1.4, background: 'var(--bg2)', border: '1px solid var(--bd)', borderRadius: 14, padding: '20px 22px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <span style={{ fontSize: 12, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: pct >= 0 ? 'rgba(74,222,128,0.15)' : 'rgba(248,113,113,0.15)', color: pct >= 0 ? '#4ADE80' : '#F87171' }}>{fmtPct(pct)}</span>
                <span style={{ fontSize: 13, color: 'var(--t3)' }}>P/L : {pl >= 0 ? '+' : ''}{pl.toFixed(0)} EUR</span>
              </div>
              <div style={{ height: 140 }}>
                <PatrimoineChart period={period} />
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: 190 }}>
              <div style={{ flex: 1, background: 'var(--bg2)', border: '1px solid var(--bd)', borderRadius: 12, padding: '14px 16px' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 3 }}>Cout de revient</div>
              </div>
              <div style={{ flex: 1, background: 'var(--bg2)', border: '1px solid var(--bd)', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '.5px', color: 'var(--t3)', marginBottom: 6 }}>PLUS-VALUE</div>
                <div style={{ fontSize: 22, fontWeight: 600, color: pl >= 0 ? '#4ADE80' : '#F87171' }}>{pl >= 0 ? '+' : ''}{pl.toFixed(0)} EUR</div>
                <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 3 }}>{fmtPct(pct)} latente</div>
              </div>
              <div style={{ flex: 1, background: 'var(--bg2)', border: '1px solid var(--bd)', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '.5px', color: 'var(--t3)', marginBottom: 6 }}>POSITIONS</div>
                <div style={{ fontSize: 22, fontWeight: 600, color: 'var(--t)' }}>{state.positions.length}</div>
                <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 3 }}>actifs suivis</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14 }}>
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--bd)', borderRadius: 14, padding: '16px 20px' }}>
              ...
              {state.positions.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--t3)' }}>
                  ...
                  <div style={{ fontSize: 13, marginBottom: 14, color: 'var(--t2)' }}>Ajoutez votre premier actif</div>
                  <button onClick={() => setShowAdd(true)} style={{ fontSize: 12, color: '#4ADE80', background: 'rgba(74,222,128,0.05)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: 8, padding: '7px 14px', cursor: 'pointer' }}>+ Ajouter</button>
                </div>
              ) : (
                state.positions.slice(0, 5).map((p, i) => {
                  ...
                  return (
                    <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '1px solid var(--bd)' }}>
                      <div style={{ width: 30, height: 30, borderRadius: 8, background: color + '18', color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, flexShrink: 0 }}>{p.nom.slice(0, 2).toUpperCase()}</div>
            ...
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--bd)', borderRadius: 14, padding: '16px 20px' }}>
              <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '.4px', color: 'var(--t2)', marginBottom: 14 }}>REPARTITION</div>
              {state.positions.length === 0 ? (
                ...
                return Object.entries(grp).map(([type, val], i) => (
                  <div key={type} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, fontSize: 12 }}>
